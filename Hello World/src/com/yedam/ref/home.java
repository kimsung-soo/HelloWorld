	...
	public static void main(String[] args) {
		...
		while (run) {
			...
			if (selectNo == 1) { // 학생수 지정.
				...
				// "이름, 점수" 출력.
				for (int i = 0; i < scores.length; i++) {
					if(keyword.equals("") || gen ==scores[i].gender) {	//문자비교					
						System.out.printf("scores[%d]> %s %d %.1f %s\n", i, scores[i].studentName, scores[i].score,scores[i].height,scores[i].gender);
					}
				}

			} else if (selectNo == 4) { // 분석(최고큰키, 평균)
				...
				System.out.printf("학생이름:%s, 최고점수: %d\n 최고큰키:%.1f\n 평균점수: %.2f\n", maxStd.studentName, maxStd.score, maxStd.height,avg);
